...
class _EasyStepperState extends State<EasyStepper> {
  ...
  List<Widget> _buildEasySteps() {
    // Reverse the order of steps for bottom-to-top progression
    var reversedSteps = widget.steps.reversed.toList();

    return List.generate(reversedSteps.length, (index) {
      // No need to calculate the original index; use the index directly
      return widget.direction == Axis.horizontal
          ? Padding(
        padding: reversedSteps.any((element) => element.topTitle)
            ? const EdgeInsets.only(top: 10)
            : EdgeInsets.zero,
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            _buildStep(index), // Use index directly for reversedSteps
    ...
    }); // Do not reverse the list again here
  }

  BaseStep _buildStep(int index) {
    final step = widget.steps[index];
    bool isStepClickable = index == widget.activeStep ||
        (widget.reachedSteps != null && !widget.reachedSteps!.contains(index) && index <= widget.activeStep) ||
        (widget.maxReachedStep != null && index <= widget.maxReachedStep!);
    ...
    return BaseStep(
      ...
      onStepSelected: isStepClickable ? () {
        if (widget.steppingEnabled && widget.steps[index].enabled || index < _selectedIndex) {
          setState(() {
